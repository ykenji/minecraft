...
function potion_calculate(event) {
  var names = [
"Mundane Potion",
...
"Stinky Potion"
  ...
  var extended = $('input[name=extended]').is(':checked') ? 1 : 0;
  var drinkable, splash;
  if (type === 'drinkable') {
    drinkable = 1;
    splash = 0;
  } else {
  ...
  var metadata = effect | (tier << 5) | (extended << 6) | (drinkable << 13) | (splash << 14);
  $('#potion-metadata').text(metadata);

  var name = names[metadata & 0x3f];
  if (splash == 1) {
    name = 'Splash ' + name;
  }
  if (metadata == 0) {
  ...
  $('#potion-name').text(name);

  var icon = effect + '-' + splash;
  $('#potion-icon').attr('class', 'icon potions-' + version + '-' + icon);
}

$(document).ready(function() {
  var placeholder_text = 'Enter an item or block name...';
  if ($('#rows').hasClass('entities'))
    placeholder_text = 'Enter a mob or entity name...';
  ...
  if (placeholder) {
    ...
    search_field.val(placeholder_text);
  ...
  $('#potion-calculator input').change(potion_calculate);
  potion_calculate(null);
});
